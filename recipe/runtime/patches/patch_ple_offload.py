...
# This exact source is inserted into the generated vLLM worker before it
# enumerates checkpoint tensors. Keep it stdlib-only: the failure path must
# not initialise CUDA, import torch, or expose a raw-shard fallback.
PACKED_TABLE_VALIDATOR = '''def _require_packed_table(name, layer, packed_dir):
    path = os.path.join(packed_dir, f"{name}.ngram_embedding.packed_u8")
    meta_path = path + ".json"
    if not os.path.isfile(path):
        raise RuntimeError(f"PLE {name}: required packed table is missing: {path}")
    if not os.path.isfile(meta_path):
        raise RuntimeError(f"PLE {name}: required packed table sidecar is missing: {meta_path}")
    try:
        with open(meta_path, encoding="utf-8") as handle:
            meta = json.load(handle)
        rows = int(meta["total_rows"])
        width = int(meta["row_width"])
        rows_per_shard = int(meta["rows_per_shard"])
        num_shards = int(meta["num_shards"])
    except (OSError, ValueError, TypeError, KeyError, json.JSONDecodeError) as error:
        raise RuntimeError(f"PLE {name}: invalid packed-table sidecar: {error}") from error
    if width != 90 or num_shards != 128 or rows <= 0 or rows_per_shard <= 0:
        raise RuntimeError(f"PLE {name}: invalid pinned packed-table geometry")
    if rows_per_shard * num_shards != rows:
        raise RuntimeError(f"PLE {name}: inconsistent packed-table row geometry")
    if os.path.getsize(path) != rows * width:
        raise RuntimeError(f"PLE {name}: packed-table byte size does not match its geometry")
    emb = layer.ngram_embedding
    expected_width = getattr(getattr(emb, "quant_method", None), "packed_row_width", None)
    if expected_width is not None and expected_width != width:
        raise RuntimeError(f"PLE {name}: packed-table row width does not match the model")
    if emb.weight.shape[0] != rows:
        raise RuntimeError(f"PLE {name}: packed-table row count does not match the model")
    return path
'''

# The validator is also callable by its source-only tests. The generated
# worker receives the same string, rather than a second hand-maintained copy.
exec(PACKED_TABLE_VALIDATOR)
...
patch("worker.py", [
    ...
    (
        ...
        "                # Configuring PLE mmap means every PLE layer must have a\n"
        "                # valid exact table before any checkpoint shards are read.\n"
        "                path = _require_packed_table(name, layer, packed_dir)\n"
        "                packed_tables[name] = path\n"
        "                logger.info(\"PLE %s: using required packed mmap table %s\", name, path)\n"
